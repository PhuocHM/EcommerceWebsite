            ...
            <div class="col-md-9">
                <form class="form-inline my-2 my-lg-0">
                    <button style="float:right" class="btn btn-outline-success my-2 my-sm-0" type="submit">Tìm
                        kiếm</button>
                    <input style="width: 300px; margin-right: 10px; float:right" class="form-control" action="{{ route('productAttributes.index') }}" method="GET" name="productAttributes" type="text" placeholder="Tìm kiếm">
                </form>
            </div>
        </div>
        <!--end breadcrumb-->
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="d-flex align-items-center gap-3 fs-6">
                                                        ...
                                                        <a href="{{ route('productAttributes.edit', $productAttribute->id) }}" class="text-warning" data-bs-toggle="tooltip" data-bs-placement="bottom" title="" data-bs-original-title="Edit info" aria-label="Edit"><i class="bi bi-pencil-fill"></i></a>
                                                        <form action="{{ route('productAttributes.destroy', [$productAttribute->id]) }}" method="POST">
                                                            @method('DELETE')
                                                            @csrf
                                                            <a href="#" onclick="deleteProductAtrribute({{ $productAttribute->id }})" class="text-danger" data-bs-toggle="modal" data-bs-target="#deleteProductAtrribute"> <i class="bi bi-trash-fill"></i></a>
                                                        </form>
                                                    </div>
                                                </td>
                                            </tr>
